    ...
    try {
      ...
      if (user && user.email) {
        ...
        await sendTemplateEmail('kyc_submitted', user.email, {
          ...
          platformName: settings?.platformName || 'BluecrestaFx',
          supportEmail: settings?.supportEmail || 'support@BluecrestaFx.com',
    ...
    try {
      if (user && user.email) {
        ...
        const emailResult = await sendTemplateEmail('kyc_approved', user.email, {
          ...
          platformName: settings?.platformName || 'BluecrestaFx',
          loginUrl: settings?.loginUrl || 'https://BluecrestaFx.com/login',
          supportEmail: settings?.supportEmail || 'support@BluecrestaFx.com',
          year: new Date().getFullYear().toString()
        })
