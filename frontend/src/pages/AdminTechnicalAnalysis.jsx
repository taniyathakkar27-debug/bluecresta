    ...
    <AdminLayout title="Technical Analysis" subtitle="Upload and manage technical analysis for users">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-500 text-white rounded-lg hover:from-blue-700 hover:to-cyan-600 transition-colors font-medium"
          >
            <Plus size={18} />
            Add Analysis
          </button>
        ...
        {loading ? (
          ...
          <div className={`rounded-xl p-12 text-center border ${isDarkMode ? 'bg-dark-700 border-gray-800' : 'bg-white border-gray-200'}`}>
            <ImageIcon size={48} className={`mx-auto mb-4 ${isDarkMode ? 'text-gray-600' : 'text-gray-400'}`} />
            <h3 className={`text-lg font-medium mb-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>No Technical Analysis Yet</h3>
            <p className={`mb-4 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Upload your first technical analysis to notify users.</p>
            <button
              onClick={() => setShowModal(true)}
              className="px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-500 text-white rounded-lg hover:from-blue-700 hover:to-cyan-600 transition-colors font-medium"
            >
              Add Analysis
            </button>
          </div>
            ...
            {analyses.map((analysis) => (
              <div 
                ...
                className={`rounded-xl border overflow-hidden ${
                  isDarkMode
                    ? analysis.isActive ? 'bg-dark-700 border-gray-700' : 'bg-dark-700 border-gray-800 opacity-60'
                    : analysis.isActive ? 'bg-white border-gray-200' : 'bg-white border-gray-200 opacity-60'
                }`}
              >
                {/* Image */}
                <div className={`relative h-48 ${isDarkMode ? 'bg-dark-800' : 'bg-gray-100'}`}>
                  <img 
                ...
                {/* Content */}
                <div className="p-4">
                  <p className={`text-sm line-clamp-3 mb-3 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className={`rounded-xl w-full max-w-lg border max-h-[90vh] overflow-y-auto ${isDarkMode ? 'bg-dark-700 border-gray-700' : 'bg-white border-gray-200'}`}>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
                ...
                >
                  {imagePreview ? (
                    <div className="relative">
                      ...
                      />
                      <p className={`text-sm mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Click to change image</p>
                    </div>
                  ) : (
                    <div className="py-8">
                      <Upload size={32} className={`mx-auto mb-2 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
                      <p className={isDarkMode ? 'text-gray-400' : 'text-gray-500'}>Click to upload image</p>
                      <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>PNG, JPG, GIF up to 5MB</p>
                    </div>
                  )}
                </div>
                <input
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className={`w-full border rounded-lg px-4 py-2.5 focus:outline-none focus:border-accent-green resize-none ${isDarkMode ? 'bg-dark-600 border-gray-600 text-white placeholder-gray-500' : 'bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-400'}`}
                  required
                />
              </div>
              ...
              <div className="flex gap-3 pt-2">
                <button
                  type="button"
                  onClick={closeModal}
                  className={`flex-1 py-2.5 rounded-lg transition-colors ${isDarkMode ? 'bg-dark-600 text-gray-300 hover:bg-dark-500' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-gradient-to-r from-blue-600 to-cyan-500 text-white rounded-lg hover:from-blue-700 hover:to-cyan-600 transition-colors disabled:opacity-50 font-medium"
                >
                  {submitting ? (
                    <>
