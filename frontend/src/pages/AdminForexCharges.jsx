...
const AdminForexCharges = () => {
  ...
  const instrumentSelectClass = `w-full px-3 py-2 border rounded-lg text-sm ${isDarkMode ? 'bg-dark-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'}`
  ...
  return (
    <AdminLayout title="Forex Charges" subtitle="Manage trading fees and spreads">
      <div className="space-y-6">
        ...
        {/* COMMISSION SECTION */}
        <div className={`rounded-xl border ${isDarkMode ? 'bg-dark-800 border-gray-800' : 'bg-white border-gray-200'}`}>
          <div className={`flex items-center justify-between p-4 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                <DollarSign size={20} className={isDarkMode ? 'text-white' : 'text-gray-700'} />
              </div>
              <div>
                <h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Commission</h2>
                <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Trading fees per lot/trade</p>
              </div>
            </div>
            <button 
              onClick={() => { resetForm(); setEditingCharge(null); setModalType('commission') }}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${isDarkMode ? 'bg-gray-700 text-white hover:bg-gray-600' : 'bg-gray-200 text-gray-900 hover:bg-gray-300'}`}
            >
              <Plus size={16} />
          ...
          <div className="p-4">
            {loading ? (
              <p className={`text-center py-4 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Loading...</p>
            ) : charges.filter(c => c.commissionValue > 0).length === 0 ? (
              <p className={`text-center py-4 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>No commission charges configured</p>
            ) : (
              <div className="space-y-2">
                {charges.filter(c => c.commissionValue > 0).map((charge) => (
                  <div key={charge._id} className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-dark-700' : 'bg-gray-100'}`}>
                    <div className="flex items-center gap-3">
                      <span className={`px-2 py-0.5 text-xs rounded ${isDarkMode ? 'bg-gray-600 text-gray-300' : 'bg-gray-300 text-gray-700'}`}>{charge.level}</span>
                      <span className={isDarkMode ? 'text-white' : 'text-gray-900'}>{getLevelLabel(charge)}</span>
                    </div>
                    <div className="flex items-center gap-4">
                      <span className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>${charge.commissionValue} <span className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>({charge.commissionType})</span></span>
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEditModal(charge, 'commission')} className={`p-1.5 rounded ${isDarkMode ? 'hover:bg-dark-600 text-gray-400 hover:text-white' : 'hover:bg-gray-200 text-gray-600 hover:text-gray-900'}`}><Edit size={14} /></button>
                        <button onClick={() => handleDelete(charge._id)} className={`p-1.5 rounded ${isDarkMode ? 'hover:bg-dark-600 text-gray-400' : 'hover:bg-gray-200 text-gray-600'} hover:text-red-400`}><Trash2 size={14} /></button>
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <div className={`rounded-xl border ${isDarkMode ? 'bg-dark-800 border-gray-800' : 'bg-white border-gray-200'}`}>
          <div className={`flex items-center justify-between p-4 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                <TrendingUp size={20} className={isDarkMode ? 'text-white' : 'text-gray-700'} />
              </div>
              <div>
                <h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Spread</h2>
                <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Bid/Ask price difference</p>
              </div>
            </div>
            <button 
          ...
          <div className="p-4">
            {loading ? (
              ...
              <p className={`text-center py-4 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>No spread charges configured</p>
            ) : (
              <div className="space-y-2">
                {charges.filter(c => c.spreadValue > 0).map((charge) => (
                  <div key={charge._id} className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-dark-700' : 'bg-gray-100'}`}>
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{charge.spreadValue} <span className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>({charge.spreadType})</span></span>
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEditModal(charge, 'spread')} className={`p-1.5 rounded ${isDarkMode ? 'hover:bg-dark-600 text-gray-400 hover:text-white' : 'hover:bg-gray-200 text-gray-600 hover:text-gray-900'}`}><Edit size={14} /></button>
                        <button onClick={() => handleDelete(charge._id)} className={`p-1.5 rounded ${isDarkMode ? 'hover:bg-dark-600 text-gray-400' : 'hover:bg-gray-200 text-gray-600'} hover:text-red-400`}><Trash2 size={14} /></button>
                      </div>
                    </div>
                  </div>
        ...
        <div className={`rounded-xl border ${isDarkMode ? 'bg-dark-800 border-gray-800' : 'bg-white border-gray-200'}`}>
          <div className={`flex items-center justify-between p-4 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${isDarkMode ? 'bg-gray-700' : 'bg-gray-200'}`}>
                <Moon size={20} className={isDarkMode ? 'text-white' : 'text-gray-700'} />
              </div>
              <div>
                <h2 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Swap</h2>
                <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Overnight holding fees</p>
              </div>
            </div>
            <button 
          ...
          <div className="p-4">
            {loading ? (
              ...
              <p className={`text-center py-4 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>No swap charges configured</p>
            ) : (
              <div className="space-y-2">
                {charges.filter(c => c.swapLong !== 0 || c.swapShort !== 0).map((charge) => (
                  <div key={charge._id} className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-dark-700' : 'bg-gray-100'}`}>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-1">
                        <button onClick={() => openEditModal(charge, 'swap')} className={`p-1.5 rounded ${isDarkMode ? 'hover:bg-dark-600 text-gray-400 hover:text-white' : 'hover:bg-gray-200 text-gray-600 hover:text-gray-900'}`}><Edit size={14} /></button>
                        <button onClick={() => handleDelete(charge._id)} className={`p-1.5 rounded ${isDarkMode ? 'hover:bg-dark-600 text-gray-400' : 'hover:bg-gray-200 text-gray-600'} hover:text-red-400`}><Trash2 size={14} /></button>
                      </div>
                    </div>
                  </div>
