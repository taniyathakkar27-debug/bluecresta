  ...
  const renderModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
        <div className={`rounded-2xl w-full max-w-md border overflow-hidden ${isDarkMode ? 'bg-dark-800 border-gray-700' : 'bg-white border-gray-200'}`}>
          {/* Modal Header */}
          <div className={`flex items-center justify-between p-4 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>{selectedUser.email}</p>
              </div>
            </div>
            <button 
              onClick={closeModal}
          ...
          <div className="p-4">
              ...
              <div className="space-y-4">
                ...
                <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-dark-700' : 'bg-gray-100'}`}>
                  <div className="flex items-center gap-2 mb-1">
                    <Globe size={14} className={isDarkMode ? 'text-gray-500' : 'text-gray-600'} />
                    <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Last Login IP</p>
                  </div>
                  <p className={`font-mono text-sm ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{selectedUser.lastLoginIP || 'Never logged in'}</p>
                  {selectedUser.lastLoginAt && (
                    <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Last login: {new Date(selectedUser.lastLoginAt).toLocaleString()}</p>
                  )}
                </div>
                <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-dark-700' : 'bg-gray-100'}`}>
            ...
            {modalType === 'tradingAccounts' && (
              <div className="space-y-4">
                ...
                {userAccounts.length === 0 ? (
                  <div className={`p-4 text-center text-sm rounded-lg ${isDarkMode ? 'bg-dark-700 text-gray-500' : 'bg-gray-100 text-gray-600'}`}>
                      ...
                      <div key={acc._id} className={`rounded-lg p-4 border ${isDarkMode ? 'bg-dark-700 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                        <div className="flex items-center justify-between mb-3">
                          <div>
                            ...
                            <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>{acc.accountTypeId?.name || 'Standard'} • Leverage: {acc.leverage}</p>
                          </div>
                ...
                <div className="flex gap-2 pt-2">
                  <button 
                    ...
                    className={`flex-1 py-3 rounded-lg transition-colors ${isDarkMode ? 'bg-dark-700 text-gray-400 hover:bg-dark-600' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}
                  >
                    Back
                  </button>
                </div>
            ...
            {modalType === 'password' && (
              <div className="space-y-4">
                ...
                <div>
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>New Password</label>
                  <input
                    type="password"
                    value={newPassword}
                    onChange={(e) => setNewPassword(e.target.value)}
                    placeholder="Enter new password"
                    className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:border-blue-500 ${isDarkMode ? 'bg-dark-700 border-gray-700 text-white placeholder-gray-500' : 'bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-400'}`}
                  />
                </div>
                <div>
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Confirm Password</label>
                  <input
                    type="password"
            ...
            {modalType === 'deduct' && (
              <div className="space-y-4">
                ...
                <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-dark-700' : 'bg-gray-100'}`}>
                  <p className={`text-xs mb-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Current Wallet Balance</p>
                  <p className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>${userWalletBalance?.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2}) || '0.00'}</p>
                </div>
                <div>
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Amount to Deduct ($)</label>
                  <input
                    type="number"
                    value={deductAmount}
                    ...
                    min="0"
                    step="0.01"
                    className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:border-orange-500 ${isDarkMode ? 'bg-dark-700 border-gray-700 text-white placeholder-gray-500' : 'bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-400'}`}
                  />
                </div>
                <div>
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Reason (Optional)</label>
                  <input
                    type="text"
                    value={deductReason}
                ...
                <div>
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  ...
                  <input
                    ...
                    className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:border-orange-500 ${isDarkMode ? 'bg-dark-700 border-gray-700 text-white placeholder-gray-500' : 'bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-400'}`}
                  />
                  <p className={`text-xs mt-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Leave empty to use current date & time</p>
                </div>
                <div className="flex gap-2 pt-2">
                  <button 
                    onClick={() => setModalType('view')}
            ...
            {modalType === 'addFundWallet' && (
              <div className="space-y-4">
                ...
                <div>
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Amount to Add ($)</label>
                  <input
                    type="number"
                    value={addFundAmount}
                    onChange={(e) => setAddFundAmount(e.target.value)}
                    ...
                    min="0"
                    step="0.01"
                    className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:border-green-500 ${isDarkMode ? 'bg-dark-700 border-gray-700 text-white placeholder-gray-500' : 'bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-400'}`}
                  />
                </div>
                <div>
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Reason (Optional)</label>
            ...
            {modalType === 'addFundAccount' && (
              <div className="space-y-4">
                ...
                <div className={`p-3 rounded-lg ${isDarkMode ? 'bg-dark-700' : 'bg-gray-100'}`}>
                  <p className={`text-xs mb-1 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Selected Account</p>
                  <p className={`font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{userAccounts.find(a => a._id === selectedAccountId)?.accountId || 'N/A'}</p>
                  <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Balance: ${userAccounts.find(a => a._id === selectedAccountId)?.balance?.toFixed(2) || '0.00'}</p>
                </div>
                <div>
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Amount to Add ($)</label>
                  <input
    ...
    <AdminLayout title="User Management" subtitle="Manage all registered users">
      ...
      <div className="flex gap-2 mb-4">
        <button
          ...
          className={`px-4 py-2 rounded-lg font-medium transition-colors ${
            activeTab === 'users' ? 'bg-blue-500 text-white' : isDarkMode ? 'bg-dark-700 text-gray-400 hover:text-white' : 'bg-gray-100 text-gray-600 hover:text-gray-900'
          }`}
        >
          Users ({users.length})
        </button>
        <button
          ...
          className={`px-4 py-2 rounded-lg font-medium transition-colors flex items-center gap-2 ${
            activeTab === 'password-reset' ? 'bg-blue-500 text-white' : isDarkMode ? 'bg-dark-700 text-gray-400 hover:text-white' : 'bg-gray-100 text-gray-600 hover:text-gray-900'
          }`}
        >
          <Key size={16} />
          Password Requests
      ...
      {activeTab === 'password-reset' ? (
        <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-dark-800 border-gray-800' : 'bg-white border-gray-200'}`}>
          <div className={`flex items-center justify-between p-4 sm:p-5 border-b ${isDarkMode ? 'border-gray-800' : 'border-gray-200'}`}>
            <div>
              <h2 className={`font-semibold text-lg ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Password Reset Requests</h2>
              <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>
                Pending: {resetRequestStats.pending} | Completed: {resetRequestStats.completed} | Rejected: {resetRequestStats.rejected}
              </p>
            </div>
            <button onClick={fetchPasswordResetRequests} className={`p-2 rounded-lg ${isDarkMode ? 'bg-dark-700 hover:bg-dark-600' : 'bg-gray-100 hover:bg-gray-200'}`}>
          ...
          <div className="p-4 space-y-3">
            {passwordResetRequests.length === 0 ? (
              <p className={`text-center py-8 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>No password reset requests</p>
            ) : (
              passwordResetRequests.map(request => (
                <div key={request._id} className={`rounded-xl p-4 border ${isDarkMode ? 'bg-dark-700 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>{request.email}</p>
                        {request.newEmail && (
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <input
                              ...
                              className={`w-full border rounded-lg px-3 py-2 text-sm ${isDarkMode ? 'bg-dark-600 border-gray-600 text-white' : 'bg-gray-50 border-gray-300 text-gray-900'}`}
                            />
                          </div>
                          <div className="flex gap-2">
                            <button
                              onClick={() => { setSelectedResetRequest(null); setResetPassword('') }}
                              className={`px-4 py-2 rounded-lg text-sm ${isDarkMode ? 'bg-dark-600 text-gray-400 hover:bg-dark-500' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}
                            >
                              Cancel
                            </button>
                            <button
                  ...
                  {request.status !== 'Pending' && request.adminRemarks && (
                    <p className={`mt-2 text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Remarks: {request.adminRemarks}</p>
                  )}
                </div>
              ))
            )}
      ...
      <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-dark-800 border-gray-800' : 'bg-white border-gray-200'}`}>
        ...
        <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 sm:p-5 border-b ${isDarkMode ? 'border-gray-800' : 'border-gray-200'}`}>
          <div>
            ...
            <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>{users.length} total users</p>
          </div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className={`w-full sm:w-64 border rounded-lg pl-10 pr-4 py-2 focus:outline-none ${isDarkMode ? 'bg-dark-700 border-gray-700 text-white placeholder-gray-500 focus:border-gray-600' : 'bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-400 focus:border-gray-400'}`}
              />
            </div>
            <button 
              onClick={fetchUsers}
            ...
            >
              <RefreshCw size={18} className={`${loading ? 'animate-spin' : ''} ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`} />
              <span className={`sm:hidden text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Refresh</span>
            </button>
            <button 
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <button 
                    ...
                    className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-dark-600' : 'hover:bg-gray-200'}`}
                  >
                ...
                <div className={`flex gap-2 mt-4 pt-3 border-t ${isDarkMode ? 'border-gray-600' : 'border-gray-300'}`}>
                  <button 
                    onClick={() => openModal('view', user)}
          ...
          <table className="w-full">
            <thead>
              <tr className={`border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                ...
                <th className={`text-left text-sm font-medium py-3 px-4 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Last Login IP</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="8" className={`text-center py-8 ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>
                    {searchTerm ? 'No users found matching your search' : 'No users registered yet'}
                  </td>
                </tr>
              ) : (
                  ...
                  <tr key={user._id} className={`border-b ${isDarkMode ? 'border-gray-800 hover:bg-dark-700/50' : 'border-gray-200 hover:bg-gray-50'}`}>
                    ...
                    <td className="py-4 px-4">
                      <div className={`flex items-center gap-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                        ...
                        <div>
                          ...
                          {user.lastLoginAt && (
                            <p className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>{new Date(user.lastLoginAt).toLocaleDateString()}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-1">
                        <button 
                          onClick={() => openModal('view', user)}
                          className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-dark-600 text-gray-400 hover:text-white' : 'hover:bg-gray-200 text-gray-600 hover:text-gray-900'}`}
                          title="View Details"
                        >
                          <Eye size={16} />
                        </button>
                        <button 
                          onClick={() => openModal('password', user)}
                          className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-dark-600 text-gray-400 hover:text-blue-500' : 'hover:bg-gray-200 text-gray-600 hover:text-blue-500'}`}
                        ...
                        <button 
                          ...
                          className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-dark-600 text-gray-400 hover:text-orange-500' : 'hover:bg-gray-200 text-gray-600 hover:text-orange-500'}`}
                          title="Deduct Fund"
                        >
                          <DollarSign size={16} />
                        </button>
                        <button 
                          ...
                          className={`p-2 rounded-lg transition-colors ${
                            user.isBlocked 
                              ? 'text-yellow-500' 
                              : isDarkMode ? 'text-gray-400 hover:bg-dark-600 hover:text-yellow-500' : 'text-gray-600 hover:bg-gray-200 hover:text-yellow-500'
                          }`}
                        ...
                        <button 
                          onClick={() => openModal('delete', user)}
                          className={`p-2 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-dark-600 text-gray-400 hover:text-red-500' : 'hover:bg-gray-200 text-gray-600 hover:text-red-500'}`}
                          title="Delete User"
                        >
                          <Trash2 size={16} />
                        </button>
          ...
          <div className={`rounded-2xl w-full max-w-md border overflow-hidden ${isDarkMode ? 'bg-dark-800 border-gray-700' : 'bg-white border-gray-200'}`}>
            <div className={`flex items-center justify-between p-4 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Create New User</h3>
                  <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-600'}`}>Add a user with custom registration date</p>
                </div>
              </div>
              <button 
            ...
            <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <div>
                <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>First Name *</label>
                <input
                  type="text"
                  value={createForm.firstName}
                  onChange={(e) => setCreateForm({ ...createForm, firstName: e.target.value })}
                  placeholder="Enter first name"
                  className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:border-accent-green ${isDarkMode ? 'bg-dark-700 border-gray-700 text-white placeholder-gray-500' : 'bg-gray-50 border-gray-300 text-gray-900 placeholder-gray-400'}`}
                />
              </div>

              <div>
                <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Email *</label>
                <input
                  type="email"
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Code</label>
                  <input
                    type="text"
                ...
                <div className="col-span-2">
                  <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Phone</label>
                  <input
                    type="text"
              ...
              <div>
                <label className={`text-sm mb-1 block ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Password *</label>
                <input
                  type="password"
              ...
              <div className="flex gap-2 pt-2">
                ...
                <button 
                  ...
                  className="flex-1 py-3 bg-gradient-to-r from-blue-600 to-cyan-500 text-white rounded-lg hover:bg-green-600 transition-colors disabled:opacity-50 font-medium"
                >
