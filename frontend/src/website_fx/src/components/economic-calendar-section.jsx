    ...
    <section className="section-padding bg-[#0A0D1A]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-[#8A94B0] max-w-2xl mx-auto">
            ...
            <button
              ...
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all border ${
                filter === f
                  ...
                  : "bg-[#0F1628] text-white border-white/10 hover:bg-[#151D35]"
              }`}
            >
              {f !== "All" && (
        ...
        {/* Calendar Table */}
        <div className="bg-[#0F1628] rounded-2xl shadow-sm border border-white/8 overflow-hidden">
          {/* Desktop Header */}
          <div className="hidden md:grid grid-cols-[80px_90px_1fr_110px_100px_100px_110px] gap-4 px-6 py-3 bg-[#0A0D1A]/50 border-b border-white/8">
            {["Time", "Currency", "Event", "Impact", "Forecast", "Previous", "Actual"].map(h => (
              <span key={h} className="text-xs font-semibold text-[#8A94B0] uppercase tracking-wide">{h}</span>
            ))}
          </div>
          ...
          <div className="divide-y divide-white/8">
            {filtered.map((event, i) => (
              <div key={i}>
                {/* Desktop Row */}
                <div className="hidden md:grid grid-cols-[80px_90px_1fr_110px_100px_100px_110px] gap-4 items-center px-6 py-4 hover:bg-[#151D35] transition-colors">
                  <div className="flex items-center gap-1.5 text-sm font-mono text-white">
                    <Clock className="w-3.5 h-3.5 text-[#8A94B0]" />
                    {event.time}
                  </div>
                  ...
                  <span className="text-sm text-[#8A94B0] font-mono">{event.forecast}</span>
                  <span className="text-sm text-[#8A94B0] font-mono">{event.previous}</span>
                ...
                <div className="md:hidden">
                  <button
                    className="w-full flex items-center justify-between px-4 py-4 text-left hover:bg-[#151D35] transition-colors"
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-xs text-[#8A94B0] font-mono">{event.time}</span>
                          <span className="text-xs font-semibold text-white">{event.currency}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <ActualBadge actual={event.actual} forecast={event.forecast} />
                      <ChevronDown className={`w-4 h-4 text-[#8A94B0] transition-transform ${expanded === i ? "rotate-180" : ""}`} />
                    </div>
                  </button>
                  {expanded === i && (
                    <div className="px-4 pb-4 grid grid-cols-3 gap-3 text-center bg-[#0A0D1A]/50">
                      {[
                        ...
                        <div key={label} className="bg-[#0F1628] rounded-xl p-3 border border-white/8">
                          <div className="text-xs text-[#8A94B0] mb-1">{label}</div>
                          {value}
                        </div>
                      ))}
                    </div>
          ...
          {/* Footer */}
          <div className="bg-[#0A0D1A]/50 px-6 py-4 border-t border-white/8 flex flex-col sm:flex-row items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-4 text-xs text-[#8A94B0]">
