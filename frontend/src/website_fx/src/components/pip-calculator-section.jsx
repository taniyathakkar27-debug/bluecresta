...
export function PipCalculatorSection() {
  ...
  return (
    <section className="py-20 lg:py-28 bg-[#0A0D1A]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-5 gap-8 items-start">
          ...
          {/* Calculator Card */}
          <div className="lg:col-span-3 bg-white border border-gray-200 rounded-2xl shadow-lg overflow-hidden">
            {/* Card Header */}
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
              <div className="flex items-center gap-2">
                <Calculator className="w-5 h-5 text-primary" />
                <span className="font-semibold text-gray-900">Pip Calculator</span>
              </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-gray-900">Instrument</label>
                  <select
                    value={instrument}
                    onChange={e => setInstrument(e.target.value)}
                    className="w-full h-11 rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary/50"
                  >
                    {instruments.map(i => (
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-gray-900">Account Currency</label>
                  <select
                    value={accountCcy}
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-gray-900">Lot Type</label>
                  <select
                    value={lotSize}
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-gray-900">Number of Lots</label>
                  <input
                    type="number"
                    min="0.01"
                    step="0.01"
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-gray-900">Number of Pips</label>
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 h-11 rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary/50"
                  />
                  {/* Quick pip buttons */}
              ...
              {result && (
                <div className="grid grid-cols-1 xs:grid-cols-2 gap-3 sm:gap-4">
                  <div className="bg-primary/10 border border-primary/30 rounded-xl p-3 sm:p-4">
                    <p className="text-xs text-gray-600 mb-1 font-medium uppercase tracking-wide">Pip Value</p>
                    <p className="text-lg sm:text-2xl font-bold text-primary break-all">
                      {accountCcy} {fmt(result.pipValue)}
                    </p>
                    <p className="text-xs text-gray-600 mt-1">per pip</p>
                  </div>
                  <div className="bg-green-100 border border-green-300 rounded-xl p-3 sm:p-4">
                    <p className="text-xs text-gray-600 mb-1 font-medium uppercase tracking-wide">Total P&L</p>
                    <p className="text-lg sm:text-2xl font-bold text-green-600 break-all">
                      {accountCcy} {fmt(result.totalProfit)}
                    </p>
                    <p className="text-xs text-gray-600 mt-1">for {pips} pips</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {/* Lot Size Reference */}
            <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-lg">
              <h3 className="font-semibold text-gray-900 mb-4">Lot Size Reference</h3>
              <div className="space-y-2">
                {[
                  ...
                  <div key={row.type} className="flex items-center justify-between py-2 border-b border-gray-200 last:border-0">
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${row.color}`} />
                      <span className="text-sm font-medium text-gray-900">{row.type}</span>
                    </div>
                    <span className="text-sm text-gray-600 font-mono">{row.size}</span>
                  </div>
                ))}
              </div>
            </div>
