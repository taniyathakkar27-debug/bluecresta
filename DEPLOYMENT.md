# BluecrestaFx VPS Deployment Guide (Fresh OS Installation)
...
```bash
# Create new user
adduser BluecrestaFx

# Add user to sudo group
usermod -aG sudo BluecrestaFx
...
scp -r ./nalmi BluecrestaFx@YOUR_VPS_IP:/home/BluecrestaFx/
```

### Option C: Using SFTP/FileZilla
- Connect to VPS using FileZilla
- Upload the `nalmi` folder to `/home/BluecrestaFx/`

## Step 9: Configure Backend

```bash
cd /home/BluecrestaFx/nalmi/backend
...
PORT=5001
NODE_ENV=production
MONGODB_URI=mongodb://localhost:27017/BluecrestaFx
...
```bash
cd /home/BluecrestaFx/nalmi/frontend
...
# Start with PM2
pm2 start server.js --name BluecrestaFx-backend
...
npm install -g serve
cd /home/BluecrestaFx/nalmi/frontend
pm2 start "serve -s dist -l 5173" --name BluecrestaFx-frontend
```
...
# Configure nginx
sudo nano /etc/nginx/sites-available/BluecrestaFx
```
...
server {
    ...
    # Frontend
    location / {
        root /home/BluecrestaFx/nalmi/frontend/dist;
...
Enable the site:
```bash
sudo ln -s /etc/nginx/sites-available/BluecrestaFx /etc/nginx/sites-enabled/
sudo nginx -t
...
pm2 stop BluecrestaFx-backend   # Stop backend
pm2 delete BluecrestaFx-backend # Remove from PM2
```
...
### View logs
```bash
pm2 logs BluecrestaFx-backend --lines 100
```
